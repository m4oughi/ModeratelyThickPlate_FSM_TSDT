...
k_global(10*n+2, :) = [];    % remove U from last strip(row)
k_global(:, 10*n+2) = [];    % remove U from last strip(column)
k_global(10*n+1, :) = [];    % remove U from last strip(row)
k_global(:, 10*n+1) = [];    % remove U from last strip(column)
...
k_global(2, :) = [];        % remove U from first strip(row)
k_global(:, 2) = [];        % remove U from first strip(column)
k_global(1, :) = [];        % remove U from first strip(row)
k_global(:, 1) = [];        % remove U from first strip(column)
%}
%{
% C-C
% END
...
kg_global(10*n+2, :) = [];   % remove U from last strip(row)
kg_global(:, 10*n+2) = [];   % remove U from last strip(column)
kg_global(10*n+1, :) = [];   % remove U from last strip(row)
kg_global(:, 10*n+1) = [];   % remove U from last strip(column)
...
kg_global(2, :) = [];       % remove U from first strip(row)
kg_global(:, 2) = [];       % remove U from first strip(column)
kg_global(1, :) = [];       % remove U from first strip(row)
kg_global(:, 1) = [];       % remove U from first strip(column)
%}
%{
% C-C
% END
...
kw__global = sym(zeros(10*n+6, 10*n+6));
for j = 1:1:n
    kw___global = sym(zeros(10*n+6, 10*n+6));
    kw___global(10*j-9:10*j+6, 10*j-9:10*j+6) = kw_local;
...
ks__global = sym(zeros(10*n+6, 10*n+6));
for j = 1:1:n
    ks___global = sym(zeros(10*n+6, 10*n+6));
    ks___global(10*j-9:10*j+6, 10*j-9:10*j+6) = ks_local;
